 ...
 */
class ShadowlessClassModifier {
    private static HashSet<String> exclude = new HashSet<>();
    static {
        exclude = new HashSet<>()
//        exclude.add('android')
//        exclude.add('androidx.appcompat')
        exclude.add('com/paul/collector')
        exclude.add('com/paul/idata')
        exclude.add('com/paul/song/analytics/sdk')
    }
    ...
    private static byte[] modifyClass(byte[] srcClass) throws IOException {
        ...
        ClassVisitor classVisitor = new ShadowlessClassVisitor(classWriter)
